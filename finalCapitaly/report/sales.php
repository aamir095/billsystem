...
<body>
  ...
  <nav style="padding: 8rem; padding-top: 3rem;">
    <div class="nav nav-tabs" role="tablist">
      <a class="nav-item nav-link nav-color tab_active" href="sales.php"><i class="i_left fa fa-chart-line"></i>Sales</a>
      <a class="nav-item nav-link nav-color" href="productpurchase.php"><i class="i_left fa fa-calendar-alt"></i>Product Report</a>
      <a class="nav-item nav-link nav-color" href="Productrawmaterialpurchase.php"><i class="i_left fa fa-luggage-cart"></i>Raw Material Purchase</a>
      ...
      <div class="tab_container">
        ...
        <div class="form-row">
                  <table class="table table-striped table-sm" id="tabledata" style="margin-top: 1rem; color: white;">
                    ...
                    <tbody id="tbodydata">
                      <?php $sql = "SELECT * FROM salesbill JOIN product_item ON salesbill.id = product_item.salesbill_id ORDER BY deliverydate DESC";
...
$('.save-button').on('click', function() {
    ...
    var val2 = $.trim($('#todate').val()).replace(/ +/g, ' ').toLowerCase();
    
    $rows.show().filter(function() {
        var text1 = $(this).find('td:nth-child(3)').text().replace(/\s+/g, ' ').toLowerCase();
        var text2 = $(this).find('td:nth-child(4)').text().replace(/\s+/g, ' ').toLowerCase();
